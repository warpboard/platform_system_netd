...
int ResolverController::setDnsServers(int netid, const char* domains,
        const char** servers, int numservers) {
    if (DBG) {
        ALOGD("setDnsServers netid = %d\n", netid);
    }
...
int ResolverController::flushDnsCache(int netid) {
    if (DBG) {
        ALOGD("flushDnsCache netid = %d\n", netid);
    }
