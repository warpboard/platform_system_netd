...
/* Alphabetical */
#define ALERT_IPT_TEMPLATE "%s %s -m quota2 ! --quota %" PRIi64 " --name %s"
...
std::string BandwidthController::makeIptablesQuotaCmd(IptOp op, const char *costName, int64_t quota) {
    ...
    ALOGV("makeIptablesQuotaCmd(%d, %" PRIi64 ")", op, quota);
    ...
    asprintf(&buff, "%s bw_costly_%s -m quota2 ! --quota %" PRIi64 " --name %s", opFlag, costName, quota,
             costName);
    res = buff;
    free(buff);
    return res;
...
int BandwidthController::getInterfaceQuota(const char *costName, int64_t *bytes) {
    ...
    scanRes = fscanf(fp, "%" PRIi64, bytes);
    ALOGV("Read quota res=%d bytes=%" PRIi64, scanRes, *bytes);
    fclose(fp);
...
int BandwidthController::parseForwardChainStats(SocketClient *cli, const TetherStats filter,
    ...
    while (NULL != (buffPtr = fgets(lineBuffer, MAX_IPT_OUTPUT_LINE_LEN, fp))) {
        ...
        res = sscanf(buffPtr, "%" PRIi64 " %" PRIi64 " RETURN all -- %s %s 0.%s",
                &packets, &bytes, iface0, iface1, rest);
        ALOGV("parse res=%d iface0=<%s> iface1=<%s> pkts=%" PRIi64 " bytes=%" PRIi64 " rest=<%s> orig line=<%s>", res,
        ...
        if (filter.intIface[0] && filter.extIface[0]) {
            if (filter.intIface == iface0 && filter.extIface == iface1) {
                ALOGV("2Filter RX iface_in=%s iface_out=%s rx_bytes=%" PRIi64 " rx_packets=%" PRIi64 " ", iface0, iface1, bytes, packets);
                stats.rxPackets = packets;
                stats.rxBytes = bytes;
            } else if (filter.intIface == iface1 && filter.extIface == iface0) {
                ALOGV("2Filter TX iface_in=%s iface_out=%s rx_bytes=%" PRIi64 " rx_packets=%" PRIi64 " ", iface0, iface1, bytes, packets);
                stats.txPackets = packets;
                stats.txBytes = bytes;
            }
        } else if (filter.intIface[0] || filter.extIface[0]) {
            if (filter.intIface == iface0 || filter.extIface == iface1) {
                ALOGV("1Filter RX iface_in=%s iface_out=%s rx_bytes=%" PRIi64 " rx_packets=%" PRIi64 " ", iface0, iface1, bytes, packets);
                stats.intIface = iface0;
                stats.extIface = iface1;
                stats.rxPackets = packets;
                stats.rxBytes = bytes;
            } else if (filter.intIface == iface1 || filter.extIface == iface0) {
                ALOGV("1Filter TX iface_in=%s iface_out=%s rx_bytes=%" PRIi64 " rx_packets=%" PRIi64 " ", iface0, iface1, bytes, packets);
            ...
            if (!stats.intIface[0]) {
                ALOGV("0Filter RX iface_in=%s iface_out=%s rx_bytes=%" PRIi64 " rx_packets=%" PRIi64 " ", iface0, iface1, bytes, packets);
                stats.intIface = iface0;
                stats.extIface = iface1;
                stats.rxPackets = packets;
                stats.rxBytes = bytes;
            } else if (stats.intIface == iface1 && stats.extIface == iface0) {
                ALOGV("0Filter TX iface_in=%s iface_out=%s rx_bytes=%" PRIi64 " rx_packets=%" PRIi64 " ", iface0, iface1, bytes, packets);
                stats.txPackets = packets;
                stats.txBytes = bytes;
            }
        }
        if (stats.rxBytes != -1 && stats.txBytes != -1) {
            ALOGV("rx_bytes=%" PRIi64 " tx_bytes=%" PRIi64 " filterPair=%d", stats.rxBytes, stats.txBytes, filterPair);
...
char *BandwidthController::TetherStats::getStatsLine(void) const {
    char *msg;
    asprintf(&msg, "%s %s %" PRIi64 " %" PRIi64 " %" PRIi64 " %" PRIi64, intIface.c_str(), extIface.c_str(),
